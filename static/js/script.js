document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleFetchError(response, error, message) {
        ...
        if (error.message === 'Failed to fetch' || error.message.startsWith('Network')) {
            // **Important:** Only redirect if we're not already on the dashboard
    ...
    function updateUI(data) {
        if (!data) {
            console.error("Received undefined data in updateUI");
            return;
        }
        ...
        if (data.processes) { // Check if data.processes exists
        ...
        if (data.network_interfaces && data.network_interfaces.interfaces) { // Check if data.network_interfaces and interfaces exist
